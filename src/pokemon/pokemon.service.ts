...
export class PokemonService {
    ...
    return this.httpService
      ...
      .pipe(
        mergeMap((response: AxiosResponse) =>
          forkJoin(response.data.results.map((item) => this.httpService.get(item.url))).pipe(
  ...
  async getPokemonById(id: number): Promise<IPokemonDetails> {
    try {
      ...
      const appropriateAbilitiesData = abilitiesDetailsResponses.map((abilityItem) => ({
        title: abilityItem.name,
        description: abilityItem.effect_entries.find((effect) => effect.language.name === 'en')
          .effect,
      }));
